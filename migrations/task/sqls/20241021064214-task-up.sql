...
-- 5-5. 修改：`王小明` 現在已經加入直播室了，請在`COURSE_BOOKING`更新該筆預約資料（請注意，不要更新到已經取消的紀錄）：
    -- 1. 請在該筆預約記錄他的加入直播室時間 `join_at` 設為 
    -- 2. 狀態`status` 設定為上課中
UPDATE "COURSE_BOOKING" 
...
SELECT 
	...
	sum("CREDIT_PURCHASE".purchased_credits) - (SELECT count(*) FROM "COURSE_BOOKING" WHERE user_id = (SELECT id FROM "USER" WHERE email = 'wXlTq@hexschooltest.io') AND status = '上課中') as remaining_credit
FROM "CREDIT_PURCHASE"
WHERE user_id = (SELECT id FROM "USER" WHERE email = 'wXlTq@hexschooltest.io')
...
FROM "CREDIT_PURCHASE"
WHERE "CREDIT_PURCHASE".purchase_at >= '2024-12-01 00:00:00' and "CREDIT_PURCHASE".purchase_at <= '2024-12-31 23:59:59';
